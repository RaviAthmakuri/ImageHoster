...
public class ImageRepository {
    ...
    public Image getImageByTitle(String title,Integer imageId) {
        EntityManager em = emf.createEntityManager();
        try {
            TypedQuery<Image> typedQuery = em.createQuery("SELECT i from Image i where i.title =:title and i.id = :imageId", Image.class).setParameter("title", title).setParameter("imageId",imageId);
